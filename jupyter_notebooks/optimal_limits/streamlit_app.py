...
with col2:
    ...
    choices = german_stocks

    def format_choice(choice):
        return choice.get("name", "Unknown")

    selected_stock = st.selectbox("Which stock do you want to analyze", choices, format_func=format_choice)
...
if trend[0]:
    ...
    with st.container():
        ...
        # Results as Table
        styler = result_df[result_df["Optimal buy limit"].isnull() == False].style.hide().format('{0:,.2f} €', subset=['Trendbased price', 'Optimal buy limit', 'Optimal sell limit'], decimal=',', ...)
